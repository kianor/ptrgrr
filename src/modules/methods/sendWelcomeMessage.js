    ...
    if (config.Join.Messages.Enabled) {
        ...
        if (config.Cards.Welcome.Enabled && Canvas) {
            ...
            config.Cards.Welcome.Lines.forEach((line, i) => {
                ...
                if (line.Size == "best-fit") ctx.font = `${getSize(text, line.StartingSize)}px "FONT"`;
                else if (parseInt(line.Size)) ctx.font = `${parseInt(line.Size)}px "FONT"`;
                else {
            ...
            return channel.send({ files: [new Utils.Discord.MessageAttachment(await canvas.toBuffer(), "welcome.png")] });
        }
