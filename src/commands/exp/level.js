    ...
    run: async (bot, messageOrInteraction, args, { member, user, guild, reply, prefixUsed }) => {
        return new Promise(async resolve => {
            ...
            if (config.Cards.Level.Enabled && Canvas) {
                ...
                ctx.font = `${getSize(mentionedUser.user.tag.toUpperCase())}px "FONT"`;
                ctx.textAlign = "center";
                ctx.fillText(member.user.tag.toUpperCase(), 650, 80);
                ...
                // Write relative XP
                ctx.font = `${getSize(`${currentXPRelative.toLocaleString()} / ${xpNeededRelative.toLocaleString()}`)}px "FONT"`;
                ctx.textAlign = "center";
                ctx.fillText(`${currentXPRelative.toLocaleString()} / ${xpNeededRelative.toLocaleString()}`, 634, 226);
    
                // Write user's total XP
                ctx.font = `${getSize(config.Cards.Level.Text.XP.replace(/{total-xp}/g, (xp.xp || 0).toLocaleString()))}px "FONT"`;
                ctx.fillStyle = "#ffffff";
