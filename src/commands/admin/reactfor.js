                ...
                if (t == "specific") {
                    ...
                    while (!valid) {
                        await Utils.waitForResponse(member.id, channel)
                            .then(res => {
                                ...
                                let position = apps.Positions[Object.keys(apps.Positions).find(p => p.toLowerCase() == res.content.toLowerCase())];
                    ...
                    let buttons = Object.keys(apps.Positions).map((position, index) => {
                        ...
                        return {
                            ID: "reactfor_application_" + position,
                            Style: buttonSettings && buttonSettings.Style ? buttonSettings.Style : "PRIMARY",
                            Emoji: buttonSettings && buttonSettings.Emoji && buttonSettings.Emoji.Enabled ? buttonSettings.Emoji.Emoji : { Enabled: true, Emoji: getEmoji(index + 1) },
