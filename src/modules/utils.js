...
module.exports = {
                    ...
                    row.slice(0, 5).map(buttonID => module.exports.variables.buttons.Buttons.find(b => "custombutton_" + b.ID == buttonID) || buttonID).filter(b => b && (b.ID || b.URL) && b.Label && b.Style).forEach(button => {
                        ...
                        if (Variables && typeof Variables === 'object') {
                            Variables.forEach(v => {
                                ...
                                if (Style) Style = Style.replace(v.searchFor, v.replaceWith);
    ...
    DDHHMMSSfromMS(ms, abbreviated = true) {
        ...
        if (days > 0)
            total.push(~~days + " day") + (days == 1 ? "" : "s");
        if (hours > 0)
            total.push(~~hours + (abbreviated ? " hr" : "hour") + (hours == 1 ? "" : "s"));
        if (minutes > 0)
            total.push(~~minutes + (abbreviated ? " min" : " minute") + (minutes == 1 ? "" : "s"));
        if (secs > 0)
            total.push(~~secs + (abbreviated ? " sec" : " second") + (secs == 1 ? "" : "s"));
        if ([~~days, ~~hours, ~~minutes, ~~secs].every(time => time == 0)) total.push("0 " + (abbreviated ? "secs" : "seconds"));
        if (!abbreviated) total[total.length - 1] = "and " + total[total.length - 1];
        return total.join(", ");
    },
    color: {
        "Reset": "\x1b[0m",
