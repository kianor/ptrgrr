        ...
        (channel.id == message.channel.id ? message.edit(embed) : channel.send(embed))
            .then(async msg => {
                ...
                if (config.Suggestions.NotifyUserOnStatusChange) {
                    creator.send(Embed({
                        title: status == "pending" ? lang.Other.SuggestionStatusChanged.Pending : lang.Other.SuggestionStatusChanged.Other.replace(/{status}/g, status),
                ...
                if (config.Suggestions.AddManagementReactions) {
                    ["Accepted", "Denied", "Implemented", "Pending"].filter(type => proper !== type).forEach(async type => {
