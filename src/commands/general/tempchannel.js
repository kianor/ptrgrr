...
module.exports = {
    ...
    run: async (bot, messageOrInteraction, args, { user, member, guild, channel, reply }) => {
        return new Promise(async resolve => {
            let tempChannel = Utils.variables.tempChannels.get(user.id);

            if (!tempChannel || (tempChannel && !guild.channels.cache.get(tempChannel.channel.id))) {
            ...
            let getFields = async () => {
                let tempChannel = await Utils.variables.tempChannels.get(user.id);
                ...
                let PrivateStatus = tempChannel.public
                    ...
                    : [
                        ...
                        },
                        {
                            name: managerLang
                                .Fields[2],
                            value: tempChannel.allowedUsers
                                .map((id) => "<@" + id + ">")
                                .join(", "),
                            inline: true,
                        },
                ...
                return [
                    {
                        ...
                        value: tempChannel.channel.name,
                        inline: true,
                    },
                    ...PrivateStatus,
                    {
                        name: managerLang.Fields[3],
                        value: tempChannel.maxMembers
                            ? tempChannel.maxMembers
            ...
            reply(msg).then(msg => {
                ...
                let checkChannel = async () => {
                    ...
                    let channel = guild.channels.cache.get(tempChannel.channel.id);
                ...
                collector.on('collect', async interaction => {
                    let voiceChannel = guild.channels.cache.get(tempChannel.channel.id);
                    ...
                    if (interaction.customId.endsWith("transfer")) { // Transfer Ownership
                        ...
                        Utils.variables.tempChannels.delete(user.id);
                        ...
                        voiceChannel.members.filter(member => !tempChannel.allowedUsers.includes(member.id) && member.id !== user.id).forEach(member => {
                                    ...
                                    while (retry) {
                                        await Utils.waitForResponse(user.id, channel).then(async res => {
                                            ...
                                            if (res.mentions.users.size) {
                                                retry = false;
                                                mesg.delete();
                                                res.mentions.users = res.mentions.users.filter(u => !tempChannel.allowedUsers.includes(u.id) || u.id == user.id);
                                                if (!res.mentions.users.size) {
                                                    retry = false;
                                                    announce = false;
                                    ...
                                    while (retry) {
                                        await Utils.waitForResponse(user.id, channel).then(async res => {
                                            ...
                                            if (res.mentions.users.size) {
                                                retry = false;
                                                mesg.delete();
                                                res.mentions.users = res.mentions.users.filter(user => ![bot.user.id, user.id].includes(user.id) && tempChannel.allowedUsers.includes(user.id));
                                                if (!res.mentions.users.size) {
                                                    retry = false;
                                                    announce = false;
