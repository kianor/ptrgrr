    ...
    run: async (bot, messageOrInteraction, args, { member, user, guild, channel, reply }) => {
        return new Promise(async resolve => {
            ...
            const finishAnnouncement = async () => {
                if (toTag && typeof toTag == 'string') answers[2].send(toTag).then(m => Utils.delete(m, 1500));
                if (toTag && typeof toTag == 'object' && toTag.length > 0) answers[2].send(toTag.map(id => '<@&' + id + '>').join(', ')).then(m => Utils.delete(m, 1500));
                ...
                if (answers[2] instanceof Collection) {
                    answers[2].forEach(mentionedChannel => mentionedChannel.send(embed));
                } else answers[2].send(embed);
